var results_partial_daily = (function Results() {
	var ROWS_TO_JUMP = 3;
	...
	function processData(data) {
		...
		function _appendPrevInfo(what) {
			/** Hardcoded processment to put some part of the 'what' im bolder **/
			var tokens = what.split(",");
			if (tokens.length > 1) {
				var moreTokens = tokens[tokens.length - 1].split(":");
				var toBeBolded = moreTokens.slice(1).join(":");
				var elem = $(document.createElement("span")).addClass("bolder orange").html(toBeBolded);
				$("#prev-info .info-wrapper").append($(document.createElement("div"))
										.attr("class", "info-elem"))
										.append(tokens[0] + ", " + moreTokens[0] + ": ")
										.append(elem);
			} else {
				tokens = what.split(":");
		...
		postprocessTable();
		$("#results").tablesorter({
			sortList: [[PERCENT_FAILED_COLUMN, 1]],
			dateFormat: "dd/mm/yy",
			debug: true,
		}); // {sortList: [[0,0], [1,0]]} 
	}
